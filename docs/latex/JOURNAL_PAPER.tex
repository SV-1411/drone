...
\begin{abstract}
Most crimes against women happen in places where existing safety technology
fails: dark streets, forest paths, and no-signal parking areas. Current
solutions put the burden of safety on the potential victim. She must carry a
device, install an application, keep it charged, and reach for it during an
attack. VanniKawachh removes that burden. Solar-powered
...
carrying the node's surveyed GPS coordinates, travel over LoRa to a police
dashboard with no SIM card or cellular coverage needed. The same alert
auto-dispatches a Pixhawk quadcopter that records evidence at the scene and
delivers a first-aid kit. A safety framework protects the response chain:
telemetry-verified flight-mode delivery, a debounced severity-ordered
failsafe arbiter, and a landing-interlocked mission queue. We
...
\IEEEPARstart{P}{ublic}-safety statistics and case studies show that violent
crimes against women concentrate in locations with poor infrastructure
coverage: unlit streets, isolated stretches between transit stops, forest and
campus peripheries, and underground or remote parking areas. These are also
the locations where the two main classes of safety technology break down.
Cellular applications lose connectivity. Camera surveillance needs lighting,
power, and a human observer. Personal devices require the victim to possess,
charge, and physically operate them during an assault.

The last point is a structural flaw rather than an engineering one. Nearly
every deployed women-safety solution (smartphone applications, smart
wearables, GPS panic buttons) uses a \emph{victim-carried} trigger. The
system works only if the woman (i)~owns the device, (ii)~is carrying it,
(iii)~has kept it charged, (iv)~has network coverage, and (v)~can reach and
operate it during the seconds in which an attack unfolds. Each condition
sounds reasonable on its own; together they are fragile. Reported detection
accuracies of such devices exceed 97\%~\cite{bharathi2025}, but the accuracy
holds only while the device is present and usable. The threat model itself
undermines that condition, since an assailant's first action is often to
separate the victim from her phone.

VanniKawachh (``Vanni'' means voice; ``Kawachh'' means shield) moves the
responsibility off the victim. The sensing infrastructure is mounted on
public poles, powered by solar energy, and listens continuously; the victim
carries nothing. The trigger is her voice: a scream, a cry, or the keywords
``help'' or ``bachao''. Fixed and aerial infrastructure performs the
detection, verification, alerting, and physical first response.
...
protection. The packet fits in a single LoRa frame, and the design starts
from the fact that a spoofed packet would launch an aircraft.
...
simulated first-aid drop), together with a plain statement of which
...
three-tier architecture. Sections~\ref{sec:acoustic} to~\ref{sec:safety} detail
...
response time, and a 1.92\% false-positive rate. These figures are strong,
but they apply only while the wearable is worn and charged. Snehith
...
tracker, which does not work in the GSM dead zones where risk is
...
order of magnitude. However, the model is compute-heavy and cannot be
deployed on low-power field nodes. Sharma and Jebaseeli~\cite{sharma2025}
report a scream-and-panic detector at 92\% accuracy with under 5\% false
positives and 4 to 6~s response, bound to the user's device. Srimathi
\emph{et al.}~\cite{srimathi2025} show that scream detection can reduce
reaction time by 40\% and increase intervention rates by 30\%, which
confirms the operational value of acoustic sensing, but their system is
single-stage and offers no field response. Fime \emph{et
al.}~\cite{fime2024} achieve 95.51\% danger-sound accuracy with Noisereduce
preprocessing and InceptionV3, identifying MobileNetV2 as a lighter
alternative, but the system again stops at detection.
...
before ground units). VanniKawachh fills this gap.
...
hub, and a response drone, as shown in Fig.~\ref{fig:architecture}. All
three tiers follow one rule: no layer acts on a detection until it has
verified it, whether the thing being verified is a sound, a network packet,
or a flight-mode command.
...
verification clip, runs Stage-2 audio analysis (PANNs~\cite{kong2020}), and
fuses the result with the alert's PIR/light flags and the time of day
(Section~\ref{sec:acoustic}). Only when the configured thresholds are met
does the hub dispatch the drone, by an HTTP POST to the flight stack's
...
requirements (never miss a real event, and never launch on a false one) pull
in opposite directions and need different operating points on different
hardware.
...
\end{equation}
which mimics the ear's compressive frequency resolution. At that resolution,
screams and shouted keywords are most separable from traffic and wind.
...
set. The stage is \emph{recall-tuned}: its job is to not
...
\eqref{eq:rms} to \eqref{eq:heuristic} of Section~\ref{sec:rehearsal}) so that
...
equals 1 if $h \geq 20$ or $h < 6$ (20:00 to 06:00), else 0; $S$ is clamped to
...
a = 0.6\,c
\end{equation}
(the Stage-1 confidence, discounted) and proceeds through fusion and
...
event) is left as future work in this path.
...
0 to 255, PIR flag, LDR light level (0 to 255), node battery percentage, and
...
truncated tag with probability $2^{-64}$ per attempt, which is negligible at
LoRa frame rates. Packets with an unknown \texttt{node\_id}, a failed MAC, a
...
the demodulation threshold for the chosen SF; at SF9 to SF12 over
...
effective LoRa throughput at usable range is roughly 1 to 5.5~kbps. Applying
...
occupy the channel for over three minutes even under ideal
conditions. These numbers justify the split: the
\emph{alert} travels over LoRa with no SIM or cellular dependency, delivered
almost instantly at ranges of the 5 to 10~km class in open terrain (to be
...
In the response tier, a software error can endanger people on the ground.
The flight stack therefore treats the
autopilot as an \emph{unreliable command sink} and enforces safety as a set
of structural invariants in the companion computer, on top of
...
a human-piloted system an operator notices and retries. An autonomous
system has no operator, and the same unverified pathway carries the
...
confirmation loop bounded by a timeout (10 to 15~s): on each iteration it
...
continuous failsafe arbitration, and the bounded abort guarantee. Each gate
must pass before or during any flight activity.
...
activity}. This turns what would otherwise be an in-flight abort into a
...
mission}. The drone proceeds to RTL either way and never loiters on a failed
...
\end{aligned}
\end{equation}
The companion computer treats this estimate as the single
...
Sections~\ref{sec:acoustic} to~\ref{sec:safety}: failsafe thresholds, debounce
...
End-to-end false-positive rate on real street noise & Phases 1 to 2 & Not yet
measured \\
...
A pole-mounted microphone network raises a fair privacy concern.
VanniKawachh addresses it in the design rather than by policy. There is no
...
and its rules. Because the design collects so little, compliance becomes a
matter of process rather than of fixing over-collection.
...
beyond-visual-line-of-sight (BVLOS) response is the deployment mode of
practical interest. We describe it as a \emph{supervised pilot-program
pathway} contingent on regulatory approval (the Rules and subsequent DGCA
BVLOS experiment schemes provide such a pathway); it is not claimed as a
capability of the prototype. The design already anticipates this: the police
dashboard provides the human-supervision surface (live telemetry,
...
would migrate to the delicensed 865 to 867~MHz band~\cite{wpc865}, where
higher transmit power is permitted without an operating licence and where
Indian LoRa deployments usually operate, improving both link budget
...
VanniKawachh shows that a women-safety system does not have to place any
burden (device, charge, coverage, or action) on the woman it protects. The
system couples infrastructure-mounted two-stage acoustic sensing to an
...
queue. Software-in-the-loop validation shows the complete chain working
end-to-end, from sealed alert to first-aid drop, with 8/8 acceptance
