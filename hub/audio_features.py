...
The public API accepts arbitrary input sample rates and resamples to the
project's 16 kHz feature convention before MFCC extraction.
"""
...
def _resample(x,sr):
 if sr==DEFAULT_SR or x.size<=1:return x
 n=max(1,int(round(x.size*DEFAULT_SR/sr)))
 return np.interp(np.linspace(0.0,1.0,n),np.linspace(0.0,1.0,x.size),x).astype(np.float32)
def _frames(x,sr):
...
def modulation_roughness(audio,sr=DEFAULT_SR):
 x=_mono_float(audio); x=_resample(x,sr)
 if x.size<max(256,DEFAULT_SR//10):return 0.0
 env=np.abs(x); env-=env.mean()
 if np.allclose(env,0.0):return 0.0
 spec=np.abs(np.fft.rfft(env*np.hanning(len(env))))**2; freqs=np.fft.rfftfreq(len(env),1.0/DEFAULT_SR); band=(freqs>=ROUGHNESS_LOW_HZ)&(freqs<=min(ROUGHNESS_HIGH_HZ,DEFAULT_SR/2.0)); total=float(spec[1:].sum())+EPS
 return float(np.clip(spec[band].sum()/total,0.0,1.0))
def extract_frame_features(audio,sr=DEFAULT_SR):
 x=_mono_float(audio); x=_resample(x,sr); frames=_frames(x,DEFAULT_SR); centroid,bandwidth,rolloff,entropy,norm=_spectral(frames,DEFAULT_SR)
 return {"rms":_rms(frames),"zcr":_zcr(frames),"spectral_centroid":centroid.astype(np.float32),"spectral_bandwidth":bandwidth.astype(np.float32),"spectral_rolloff":rolloff.astype(np.float32),"spectral_entropy":entropy.astype(np.float32),"spectral_flux":_spectral_flux(norm),"f0":_f0(frames,DEFAULT_SR)}
def extract_features(audio,sr=DEFAULT_SR):
 x=_resample(_mono_float(audio),sr); frame=extract_frame_features(x,DEFAULT_SR); f0=frame["f0"]; voiced=f0[f0>0]; mf=project_mfcc(x)
