"""Clean real-mesh F450 hardware visualization.

Uses public Gazebo F450 mesh assets and keeps labels optional so the airframe
can be viewed clearly without annotation clutter.
"""
...
#app{display:grid;grid-template-columns:minmax(0,1fr) 430px;height:100vh}
#view{position:relative;min-width:0;background:#050a0f}#c{width:100%;height:100%;display:block}
.top{position:absolute;z-index:20;left:13px;right:13px;top:13px;display:flex;justify-content:space-between;pointer-events:none}
.pill{background:#07131eed;border:1px solid var(--line);border-radius:999px;padding:9px 12px}.pill b{color:#fff}
#labels{position:absolute;inset:0;pointer-events:none;transition:opacity .2s}.labels-hidden{opacity:0}
.tag{padding:5px 7px;border:1px solid currentColor;border-radius:6px;background:#061017ee;font-size:10px;white-space:nowrap;box-shadow:0 5px 12px #0007}.tag small{display:block;color:#a5bac4;font-size:8px;margin-top:2px}
#side{overflow:auto;background:#07121bf8;border-left:1px solid var(--line);padding:13px}.title{font-size:22px;font-weight:900}.sub,.note{font-size:10px;color:var(--muted);line-height:1.55}
.card{background:#0b1821;border:1px solid var(--line);border-radius:12px;padding:11px;margin:9px 0}.lab{font-size:10px;letter-spacing:.1em;color:var(--muted);font-weight:900}.big{font-size:27px;font-weight:900}.ok{color:var(--ctrl)}.warn{color:var(--power)}
.row{display:flex;justify-content:space-between;gap:10px;margin:7px 0}.val{font-weight:900;text-align:right}.seq{display:grid;gap:4px}.step{padding:7px 9px;border:1px solid #1d3440;border-radius:8px;color:#667f8c}.step.done{background:#0c211b;color:#b0f4e2;border-color:#2a705e}.step.active{background:#231a0f;color:#ffe0a1;border-color:#936827}
.chain{display:grid;grid-template-columns:1fr auto 1fr auto 1fr auto 1fr;gap:3px;align-items:center}.node{padding:7px 3px;border:1px solid #294653;border-radius:7px;text-align:center;font-size:8px}.node.active{border-color:var(--ctrl);box-shadow:0 0 14px #39e6b833}
.btns{display:grid;grid-template-columns:1fr 1fr;gap:7px}.btn{border:1px solid var(--line);border-radius:8px;padding:10px;background:#122532;color:#fff;font-weight:900;cursor:pointer}.btn.primary{background:#0c3026;border-color:#2c705e}.btn.warn{background:#2a2113;border-color:#75552b}.btn:disabled{opacity:.45}
.status{padding:9px;border:1px solid var(--line);border-radius:8px;background:#061016;font-size:10px;margin:8px 0}.good{border-color:#2a705e;color:#b4f5e4}.bad{border-color:#913443;color:#ffc0c7}.log{height:88px;overflow:auto;background:#051017;border:1px solid #17303d;border-radius:7px;padding:6px;font:9px/1.45 monospace;color:#8fb9c7}
...
<div id="app">
<section id="view">
<div class="top"><div class="pill"><b>VANNIKAWACHH</b> · REAL F450 / PIXHAWK HARDWARE</div><div class="pill">SOURCE: <b id="source">LOADING F450 MESHES</b></div></div>
<canvas id="c"></canvas><div id="labels"></div>
</section>
<aside id="side">
<div class="title">Real F450 Hardware Assembly</div>
<div class="sub">Real F450 Gazebo meshes with the proposed Pixhawk power, control and payload hardware overlaid. Use <b>HIDE LABELS</b> for an unobstructed aircraft view.</div>
...
<div class="btns"><button id="live" class="btn primary">LIVE HUB</button><button id="test" class="btn">TEST DISTRESS</button><button id="labelsBtn" class="btn warn">HIDE LABELS</button><button id="explode" class="btn">EXPLODED VIEW</button><button id="assemble" class="btn">ASSEMBLED VIEW</button></div>
...
const labels=new CSS2DRenderer();labels.domElement.style.position='absolute';labels.domElement.style.inset='0';labels.domElement.style.pointerEvents='none';$('labels').appendChild(labels.domElement);
...
scene.add(new THREE.HemisphereLight(0xdbefff,0x11151a,2));const key=new THREE.DirectionalLight(0xffffff,3);key.position.set(3,5,4);key.castShadow=true;scene.add(key);scene.add(new THREE.GridHelper(8,16,0x173640,0x0c242d));
...
function box(s,m,pos,title,sub,ex,color){const o=new THREE.Mesh(new THREE.BoxGeometry(...s),m);o.position.copy(pos);o.userData.base=pos.clone();o.userData.ex=ex.clone();drone.add(o);liveParts.push(o);tag(o,title,sub,color);return o}
const fc=box([.36,.06,.24],M.blue,new THREE.Vector3(0,.27,0),'PIXHAWK / CUBE','ArduPilot FC',new THREE.Vector3(0,1.55,0),'#39e6b8');
const gps=box([.18,.04,.18],M.white,new THREE.Vector3(0,.48,.02),'GPS + COMPASS','F9P / RTK',new THREE.Vector3(0,1.9,.2),'#69baff');
const telem=box([.22,.05,.14],M.red,new THREE.Vector3(-.34,.34,0),'915 MHz TELEMETRY','companion / link',new THREE.Vector3(-1.5,1.3,0),'#69baff');
const pdb=box([.28,.05,.18],M.gold,new THREE.Vector3(0,.02,0),'POWER MODULE / PDB','battery distribution',new THREE.Vector3(-1.2,.3,0),'#ffb34c');
const servo=box([.18,.06,.12],M.pay,new THREE.Vector3(.36,.02,.02),'PAYLOAD SERVO','release actuator',new THREE.Vector3(1.4,.4,0),'#ff6878');
const bay=box([.42,.10,.36],M.gold,new THREE.Vector3(0,-.15,.38),'PAYLOAD BAY','health kit',new THREE.Vector3(1.4,-.3,.9),'#ff6878');
function setExploded(on){exploded=on;liveParts.forEach(o=>{if(o.userData.base&&o.userData.ex)o.position.copy(on?o.userData.ex:o.userData.base)});rotors.forEach(o=>{if(o.userData.base&&o.userData.ex)o.position.copy(on?o.userData.ex:o.userData.base)});$('explode').textContent=on?'ASSEMBLED VIEW':'EXPLODED VIEW'}
$('labelsBtn').onclick=()=>{const hidden=$('labels').classList.toggle('labels-hidden');$('labelsBtn').textContent=hidden?'SHOW LABELS':'HIDE LABELS'};
$('explode').onclick=()=>setExploded(!exploded);$('assemble').onclick=()=>setExploded(false);
$('live').onclick=()=>{log('LIVE HUB refresh');poll()};
$('test').onclick=async()=>{const b=$('test');b.disabled=true;setStatus('Sending distress…');try{const r=await fetch('/node-alert?node=WEB-F450-REAL&lat=21.128&lon=79.047&event=1&conf=0.99&pir=1&light=30');if(!r.ok)throw Error('HTTP '+r.status);log('DISTRESS → accepted');setStatus('Distress accepted','good')}catch(e){log('TRIGGER ERROR → '+e.message);setStatus('Trigger failed: '+e.message,true)}finally{setTimeout(()=>b.disabled=false,600)}};
function apply(d){state=d.state||'IDLE';rpm=Number(d.motor_rpm||0);$('state').textContent=state;$('mission').textContent=d.mission_id||'—';$('speed').textContent=Number(d.speed_ms||0).toFixed(1)+' m/s';$('alt').textContent=Number(d.altitude_m||0).toFixed(1)+' m';$('rpm').textContent=Math.round(rpm);const active=['ARMING','TAKEOFF','ENROUTE','HOVERING','DELIVERING','RTL','LANDING'].includes(state);$('powerState').textContent=active?'POWERED':'READY';$('motorState').textContent=active?'RUNNING':'OFF';$('link').textContent=state==='IDLE'?'STANDBY':'ACTIVE';$('ctrl').textContent=active?'PWM / DShot':'STANDBY';$('servo').textContent=state==='DELIVERING'?'OPEN':'CLOSED';['n1','n2','n3','n4'].forEach(x=>$(x).classList.remove('active'));if(state!=='IDLE'){$('n1').classList.add('active');$('n2').classList.add('active')}if(active)$('n3').classList.add('active');if(['DELIVERING','RTL','LANDING','COMPLETED'].includes(state))$('n4').classList.add('active');const steps={s1:['ARMING','TAKEOFF','ENROUTE','HOVERING','DELIVERING','RTL','LANDING','COMPLETED'],s2:['ARMING','TAKEOFF','ENROUTE','HOVERING','DELIVERING','RTL','LANDING','COMPLETED'],s3:['TAKEOFF','ENROUTE','HOVERING','DELIVERING','RTL','LANDING','COMPLETED'],s4:['TAKEOFF','ENROUTE','HOVERING','DELIVERING','RTL','LANDING','COMPLETED'],s5:['ENROUTE','HOVERING','DELIVERING','RTL','LANDING','COMPLETED'],s6:['HOVERING','DELIVERING','RTL','LANDING','COMPLETED'],s7:['DELIVERING','RTL','LANDING','COMPLETED'],s8:['RTL','LANDING','COMPLETED'],s9:['RTL','LANDING','COMPLETED']};Object.keys(steps).forEach(id=>$(id).className='step '+(steps[id].includes(state)?'done':''));if(state!==lastState){log('STATE → '+state);lastState=state} }
...
addEventListener('resize',resize);resize();setExploded(false);poll();setStatus('Loading real F450 mesh assembly…');
const clock=new THREE.Clock();function animate(){requestAnimationFrame(animate);const dt=clock.getDelta();const spin=rpm*Math.PI*2/60*dt*.75;rotors.forEach(r=>r.rotation.z+=spin*(r.userData.dir||1));controls.update();renderer.render(scene,cam);labels.render(scene,cam)}animate();
</script>
</body></html>'''
