...
from .distress_classifier import CLASS_NAMES, DistressClassifier, build_feature_vector
...
def main() -> int:
    ...
    for path, label in collect(args.dataset):
        ...
        if rep is None:
            rep = detector.class_score_vector(audio, sr)
        pred = model.predict_features(build_feature_vector(audio, sr, rep))
        y_true.append(label); y_pred.append(pred.predicted_class)
    ...
    fn = int(np.sum(truth & (~pred))); tp = int(np.sum(truth & pred))
