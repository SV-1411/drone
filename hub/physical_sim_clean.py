...
class PhysicalSimDrone:
    def __init__(self,lat,lon,speed_ms=15.0,name="Base"):
        ...
        self.altitude_m=0.0; self.ground_speed_ms=0.0; self.heading_deg=0.0; self.target=None; self.kit_dropped=False
    ...
    def dispatch(self,lat,lon,priority="high",node_name=""):
        with self.lock:
            ...
            self.kit_dropped=False; self.started_at=time.time(); self.state="ARMING"; self.ground_speed_ms=0; self.altitude_m=0; self.heading_deg=bearing(self.base,(float(lat),float(lon))); self.distance_m=haversine_m(self.base,(float(lat),float(lon))); self.eta_reach_s=self.distance_m/self.speed+14; self.battery_pct=100
    ...
    def _climb(self,g,target,seconds):
        start=self.altitude_m; t=time.time()
        while True:
            f=min(1,(time.time()-t)/seconds); self._set(g,altitude_m=start+(target-start)*f,ground_speed_ms=0)
            if f>=1:return
            time.sleep(.05)
    def _travel(self,g,start,end,seconds,state,alt):
        ...
        while True:
            ...
            self._set(g,lat=lat,lon=lon,altitude_m=alt,ground_speed_ms=self.speed,heading_deg=bearing(start,end),state=state,distance_m=left,eta_reach_s=left/self.speed)
            if f>=1:return
            time.sleep(.05)
    def _run(self,g):
        ...
        try:
            ...
            if not self._active(g):return
            self._set(g,state="HOVERING",ground_speed_ms=0,distance_m=0,eta_reach_s=0); time.sleep(3)
            if not self._active(g):return
            self._set(g,state="DELIVERING",ground_speed_ms=0); time.sleep(2); self._set(g,kit_dropped=True); time.sleep(1)
            ...
            self._set(g,state="COMPLETED",lat=self.base[0],lon=self.base[1],altitude_m=0,ground_speed_ms=0,distance_m=0,eta_reach_s=0)
        except Exception:self._set(g,state="FAILED")
