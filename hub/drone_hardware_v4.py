...
const scene=new T.Scene();scene.fog=new T.Fog(0x315b6d,8,28);
...
function makeGrassTerrain(groundY){const canvas=document.createElement('canvas');canvas.width=canvas.height=512;const ctx=canvas.getContext('2d');ctx.fillStyle='#315f2f';ctx.fillRect(0,0,512,512);for(let i=0;i<12000;i++){const shade=42+Math.floor(Math.random()*40);ctx.fillStyle=`rgb(${Math.floor(shade*.42)},${shade+18},${Math.floor(shade*.32)})`;ctx.fillRect(Math.random()*512,Math.random()*512,1+Math.random()*2,1+Math.random()*2)}const texture=new T.CanvasTexture(canvas);texture.colorSpace=T.SRGBColorSpace;texture.wrapS=texture.wrapT=T.RepeatWrapping;texture.repeat.set(7,7);const field=new T.Mesh(new T.CylinderGeometry(9,9,.10,72),new T.MeshLambertMaterial({color:0x35682f}));field.position.y=groundY-.055;field.receiveShadow=true;scene.add(field);const geometry=new T.PlaneGeometry(18,18,52,52);const position=geometry.attributes.position;for(let i=0;i<position.count;i++)position.setZ(i,(Math.random()-.5)*.022);position.needsUpdate=true;geometry.computeVertexNormals();const terrain=new T.Mesh(geometry,new T.MeshLambertMaterial({map:texture,color:0x78ad64,side:T.DoubleSide}));terrain.rotation.x=-Math.PI/2;terrain.position.y=groundY+.003;terrain.receiveShadow=true;scene.add(terrain)}
