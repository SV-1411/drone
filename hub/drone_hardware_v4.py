...
const scene=new T.Scene();scene.background=new T.Color(0x76acc7);scene.fog=new T.Fog(0x76acc7,10,30);
const cam=new T.PerspectiveCamera(40,1,.01,100);const renderer=new T.WebGLRenderer({canvas:$('c'),antialias:true});renderer.setPixelRatio(Math.min(devicePixelRatio,2));renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=.82;renderer.shadowMap.enabled=true;
...
function loadTerrain(groundY){new GLTFLoader().load(U+'terrain.glb',gltf=>{const terrain=gltf.scene;terrain.traverse(o=>{if(!o.isMesh)return;o.castShadow=false;o.receiveShadow=true;const material=o.material;if(material?.isMeshStandardMaterial){material.metalness=0;material.roughness=1;material.needsUpdate=true}});terrain.updateMatrixWorld(true);const rawBox=new T.Box3().setFromObject(terrain),rawSize=rawBox.getSize(new T.Vector3()),horizontal=Math.max(rawSize.x,rawSize.z)||1;terrain.scale.setScalar(14/horizontal);terrain.updateMatrixWorld(true);let box=new T.Box3().setFromObject(terrain),center=box.getCenter(new T.Vector3());terrain.position.x-=center.x;terrain.position.z-=center.z;terrain.updateMatrixWorld(true);box.setFromObject(terrain);terrain.position.y+=groundY-box.max.y;scene.add(terrain);$('source').textContent='REAL F450 + TERRAIN GLB';status('Terrain GLB loaded · F450 resting on launch field');log('Sketchfab terrain.glb loaded and aligned to landing gear')},undefined,()=>{status('Terrain GLB failed to load',true);log('Terrain failed: terrain.glb')})}
