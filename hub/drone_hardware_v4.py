...
const scene=new T.Scene();scene.background=new T.Color(0x9ebdcb);scene.fog=new T.Fog(0x9ebdcb,5,22);
...
const controls=new OrbitControls(cam,renderer.domElement);controls.enableDamping=true;controls.enablePan=true;controls.rotateSpeed=.72;controls.zoomSpeed=.85;controls.minDistance=.8;controls.maxDistance=8;
scene.add(new T.HemisphereLight(0xd8efff,0x355322,1.85));const key=new T.DirectionalLight(0xfff3d6,2.8);key.position.set(3,6,4);key.castShadow=true;key.shadow.mapSize.set(2048,2048);scene.add(key);const fill=new T.DirectionalLight(0xb5d7ff,.65);fill.position.set(-4,3,-3);scene.add(fill);
const drone=new T.Group();scene.add(drone);const overlay=new T.Group();drone.add(overlay);const parts=[],props=[],anchors=[],routes=[],pulses=[];const raycaster=new T.Raycaster(),pointer=new T.Vector2(),dragPlane=new T.Plane(),dragPoint=new T.Vector3(),dragLocal=new T.Vector3();let loaded=0,failed=0,exploded=false,rpm=0,reportedRpm=true,state='IDLE',lastState='IDLE',currentAltitude=0,targetAltitude=0,dragPart=null,dragOffset=new T.Vector3();
...
function makeGrassTerrain(groundY){const canvas=document.createElement('canvas');canvas.width=canvas.height=512;const ctx=canvas.getContext('2d');ctx.fillStyle='#527f36';ctx.fillRect(0,0,512,512);for(let i=0;i<10000;i++){const shade=42+Math.floor(Math.random()*46);ctx.fillStyle=`rgb(${Math.floor(shade*.52)},${shade+30},${Math.floor(shade*.34)})`;ctx.fillRect(Math.random()*512,Math.random()*512,1+Math.random()*2,1+Math.random()*2)}const texture=new T.CanvasTexture(canvas);texture.colorSpace=T.SRGBColorSpace;texture.wrapS=texture.wrapT=T.RepeatWrapping;texture.repeat.set(6,6);const geometry=new T.PlaneGeometry(12,12,52,52);const position=geometry.attributes.position;for(let i=0;i<position.count;i++)position.setZ(i,(Math.random()-.5)*.028);position.needsUpdate=true;geometry.computeVertexNormals();const terrain=new T.Mesh(geometry,new T.MeshStandardMaterial({map:texture,roughness:1,metalness:0,color:0xa2c77a}));terrain.rotation.x=-Math.PI/2;terrain.position.y=groundY;terrain.receiveShadow=true;scene.add(terrain)}
...
function ready(){if(loaded+failed!==10)return;const box=new T.Box3().setFromObject(drone),center=box.getCenter(new T.Vector3()),size=box.getSize(new T.Vector3()),max=Math.max(size.x,size.y,size.z)||1;drone.position.sub(center);drone.rotation.y=Math.PI;makeGrassTerrain(-size.y*.5-.012);cam.position.set(max*2.25,max*1.65,max*2.35);controls.target.set(0,0,0);controls.update();$('source').textContent=failed?'F450 PARTIAL':'REAL F450 MESH';status(failed?'F450 loaded with '+failed+' missing asset(s)':'All 10 real F450 mesh assets loaded');log('F450 assembly ready: '+loaded+'/10 real assets, heading corrected 180 degrees');makeCompactEngineeringOverlay();setExploded(false)}
...
$('live').onclick=()=>{status('Following live hub state');poll()};$('test').onclick=trigger;$('explode').onclick=()=>setExploded(!exploded);$('assemble').onclick=()=>setExploded(false);$('resetParts').onclick=resetExplodedParts;addEventListener('resize',resize);resize();poll();
let lastFrame=performance.now();function loop(now){requestAnimationFrame(loop);const dt=Math.min(.05,(now-lastFrame)/1000);lastFrame=now;const active=['ARMING','TAKEOFF','ENROUTE','HOVERING','DELIVERING','RTL','LANDING'].includes(state),effectiveRpm=active?(reportedRpm?rpm:(state==='ARMING'?1800:4300)):0;currentAltitude+=(targetAltitude-currentAltitude)*Math.min(1,dt*2.2);drone.position.y=Math.min(currentAltitude*.035,1.8);const spin=Math.min(effectiveRpm,5200)*Math.PI*2/60*dt;props.forEach(p=>{p.rotation.y+=spin*(p.userData.dir||1);p.traverse(o=>{if(o.isMesh)o.material.opacity=active?.46:.92})});parts.forEach(p=>{if(p.userData.returning)p.position.lerp(p.userData.ex,Math.min(1,dt*7));if(p.userData.returning&&p.position.distanceTo(p.userData.ex)<.002){p.position.copy(p.userData.ex);p.userData.returning=false}});pulses.forEach(p=>{p.userData.t=(p.userData.t+p.userData.speed*dt)%1;const a=exploded?(p.userData.aEx||p.userData.aBase):p.userData.aBase;const b=exploded?(p.userData.bEx||p.userData.bBase):p.userData.bBase;p.position.lerpVectors(a,b,p.userData.t);p.visible=exploded&&active});$('hud').textContent=state+' · '+(reportedRpm?Math.round(effectiveRpm):Math.round(effectiveRpm)+' EST')+' RPM · '+currentAltitude.toFixed(1)+' m AGL';controls.update();renderer.render(scene,cam);labels.render(scene,cam)}requestAnimationFrame(loop);
</script>
</body></html>'''
