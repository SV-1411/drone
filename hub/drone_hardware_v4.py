...
ASSETS = {
    ...
    "terrain.glb": "model/gltf-binary",
    "terrain-color.png": "image/png",
}
ASSET_CACHE = {}
...
<style>
:root{--bg:#0a0a0a;--panel:#f4f4f0;--line:#111;--text:#111;--muted:#666;--green:#147a3d;--blue:#145b9e;--amber:#8a5a00;--red:#b42318}
*{box-sizing:border-box}html,body{margin:0;width:100%;height:100%;overflow:hidden;background:var(--bg);color:var(--text);font:12px/1.35 Arial,Helvetica,sans-serif;letter-spacing:.01em}
#app{display:grid;grid-template-columns:minmax(0,1fr) 390px;height:100vh}#view{position:relative;min-width:0;background:#76acc7}#c{width:100%;height:100%;display:block;cursor:grab}#c.drag{cursor:grabbing}
.top{position:absolute;z-index:10;left:14px;right:14px;top:14px;display:flex;justify-content:space-between;pointer-events:none}.pill{padding:8px 10px;border:2px solid #fff;border-radius:0;background:#0a0a0a;color:#fff;font:700 10px/1 Arial,sans-serif;letter-spacing:.08em}.pill b{color:#fff}.hud{position:absolute;z-index:10;left:14px;bottom:14px;padding:8px 10px;border:2px solid #fff;background:#0a0a0a;color:#fff;font:700 10px/1.2 Consolas,monospace}.flow{position:absolute;z-index:10;left:14px;bottom:55px;padding:7px 9px;border:2px solid #145b9e;background:#fff;color:#111;font:700 9px/1.2 Consolas,monospace;display:none}
#labels{position:absolute;inset:0;pointer-events:none}#labels .tag{display:none}#labels.show .tag{display:block}.tag{padding:5px 7px;border:2px solid currentColor;border-radius:0;background:#fff;color:#111!important;box-shadow:3px 3px 0 #111;white-space:nowrap;font:700 9px Arial,sans-serif}.tag small{display:block;margin-top:2px;color:#555;font-size:8px}
#side{overflow:auto;padding:16px;background:var(--panel);border-left:3px solid #111}.title{font-size:25px;font-weight:900;letter-spacing:-.06em;text-transform:uppercase}.sub,.note{color:var(--muted);font-size:10px;line-height:1.55}.card{margin:10px 0;padding:11px;border:2px solid #111;border-radius:0;background:#fff;box-shadow:4px 4px 0 #111}.lab{color:#111;font-size:9px;font-weight:900;letter-spacing:.13em}.big{margin:3px 0;font-size:27px;font-weight:900;color:#111}.row{display:flex;justify-content:space-between;gap:10px;margin:7px 0;border-bottom:1px solid #ddd;padding-bottom:4px}.val{font-weight:900;text-align:right}.amber{color:var(--amber)}.blue{color:var(--blue)}.red{color:var(--red)}.seq{display:grid;gap:4px}.step{padding:7px 8px;border:1px solid #111;border-radius:0;color:#555;background:#fff}.step.done{background:#dff1e4;color:#111;border-color:#147a3d}.step.active{background:#fff3c4;color:#111;border-color:#8a5a00}.chain{display:grid;grid-template-columns:1fr auto 1fr auto 1fr auto 1fr;gap:3px;align-items:center}.node{padding:7px 3px;border:1px solid #111;border-radius:0;text-align:center;font-size:8px;background:#fff}.node.active{border:2px solid #147a3d;background:#dff1e4}.btns{display:grid;grid-template-columns:1fr 1fr;gap:7px}.btn{padding:10px;border:2px solid #111;border-radius:0;background:#fff;color:#111;font:900 10px Arial,sans-serif;letter-spacing:.05em;cursor:pointer;box-shadow:3px 3px 0 #111}.btn:hover{transform:translate(1px,1px);box-shadow:2px 2px 0 #111}.btn.primary{background:#111;color:#fff}.btn.warn{background:#ffe8a3}.btn:disabled{opacity:.45;transform:none;box-shadow:none}.status{margin:10px 0;padding:9px;border:2px solid #111;border-radius:0;background:#fff;font:700 10px Consolas,monospace}.good{border-color:#147a3d}.bad{border-color:#b42318;color:#b42318}.log{height:90px;overflow:auto;padding:7px;border:2px solid #111;border-radius:0;background:#111;color:#f4f4f0;font:9px/1.5 Consolas,monospace}
@media(max-width:900px){#app{grid-template-columns:1fr}#side{position:absolute;z-index:20;right:0;top:0;bottom:0;width:min(390px,96vw);box-shadow:-8px 0 0 #111}}
</style>
</head>
<body>
...
<div class="btns"><button id="live" class="btn primary">LIVE HUB</button><button id="test" class="btn">TEST DISTRESS</button><button id="explode" class="btn warn">EXPLODE VIEW</button><button id="assemble" class="btn">ASSEMBLE</button><button id="labelsBtn" class="btn" disabled>SHOW LABELS</button><button id="resetParts" class="btn">RESET PARTS</button></div><div id="status" class="status good">Loading real F450 meshes…</div><div class="note">Drag the aircraft to orbit; Shift-drag pans and the wheel zooms. In exploded mode, use SHOW LABELS to reveal the physical routes, then drag any real part freely across the inspection plane. Release it to return it smoothly to its original engineering slot. Test distress keeps the aircraft assembled and follows the same `/node-alert` → live `/drone_state` route used by the hub. Terrain model: <a href="https://sketchfab.com/3d-models/terrain-ccc6b8db791544319c4f40e1575cd715" target="_blank" rel="noreferrer">Terrain by densel</a>, CC-BY 4.0.</div><div id="log" class="log"></div></aside></div>
...
const scene=new T.Scene();scene.background=new T.Color(0x76acc7);scene.fog=new T.Fog(0x76acc7,8,22);
const cam=new T.PerspectiveCamera(40,1,.01,100);const renderer=new T.WebGLRenderer({canvas:$('c'),antialias:true});renderer.setPixelRatio(Math.min(devicePixelRatio,2));renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=.62;renderer.shadowMap.enabled=true;
...
scene.add(new T.HemisphereLight(0xc9e9ff,0x243b18,.8));const key=new T.DirectionalLight(0xffefd0,1.05);key.position.set(3,6,4);key.castShadow=true;key.shadow.mapSize.set(2048,2048);scene.add(key);const fill=new T.DirectionalLight(0xaac8e3,.18);fill.position.set(-4,3,-3);scene.add(fill);
const drone=new T.Group();scene.add(drone);const overlay=new T.Group();drone.add(overlay);const parts=[],props=[],anchors=[],routes=[],pulses=[];const raycaster=new T.Raycaster(),pointer=new T.Vector2(),dragPlane=new T.Plane(),dragPoint=new T.Vector3(),dragLocal=new T.Vector3();let loaded=0,failed=0,exploded=false,labelsVisible=false,rpm=0,reportedRpm=true,state='IDLE',lastState='IDLE',currentAltitude=0,targetAltitude=0,dragPart=null,dragOffset=new T.Vector3(),pollingStarted=false,pollInFlight=false,visualRpm=0;
...
function prop(file,sdfBase,ex,dir,index){new ColladaLoader().load(U+file,d=>{d.scene.traverse(o=>{if(o.isMesh){o.material=mats.prop;o.castShadow=o.receiveShadow=true}});const base=sdfPoint(sdfBase),compact=new T.Vector3(...ex).multiplyScalar(.52);compact.y=Math.max(compact.y,.28);const pivot=new T.Group();pivot.position.copy(base);pivot.userData.base=base.clone();pivot.userData.ex=compact;pivot.userData.dir=dir;pivot.add(d.scene);drone.add(pivot);parts.push(pivot);props.push(pivot);tag(pivot,'PROP '+index,'real Collada mesh','#dce7ed');loaded++;ready()},undefined,()=>{failed++;log('Prop failed: '+file);ready()})}
function motors(){new STLLoader().load(U+'motor_link.STL',g=>{const slots=[[[.15988,-.15988,.173],[.72,.20,.72]], [[-.15988,.15988,.173],[-.72,.20,-.72]], [[.15988,.15988,.173],[.72,.20,-.72]], [[-.15988,-.15988,.173],[-.72,.20,.72]]];slots.forEach(([sdfBase,ex],index)=>{const mesh=new T.Mesh(g,mats.black);mesh.rotation.x=-Math.PI/2;mesh.castShadow=mesh.receiveShadow=true;register(mesh,sdfPoint(sdfBase),new T.Vector3(...ex));drone.add(mesh);tag(mesh,'MOTOR '+(index+1),'real F450 motor mesh','#8de0cd')});loaded++;ready()},undefined,()=>{failed++;log('Mesh failed: motor_link.STL');ready()})}
...
function loadTerrain(groundY){const terrainMap=new T.TextureLoader().load(U+'terrain-color.png?v=4',map=>{map.colorSpace=T.SRGBColorSpace;map.flipY=false;map.anisotropy=renderer.capabilities.getMaxAnisotropy();map.needsUpdate=true},undefined,()=>{status('Terrain color texture failed to load',true);log('Terrain color texture failed: terrain-color.png')});new GLTFLoader().load(U+'terrain.glb?v=4',gltf=>{const terrain=gltf.scene;terrain.traverse(o=>{if(!o.isMesh)return;o.castShadow=false;o.receiveShadow=true;o.material=new T.MeshBasicMaterial({map:terrainMap,color:0xffffff,side:T.DoubleSide,toneMapped:false})});terrain.updateMatrixWorld(true);const rawBox=new T.Box3().setFromObject(terrain),rawSize=rawBox.getSize(new T.Vector3()),horizontal=Math.max(rawSize.x,rawSize.z)||1;terrain.scale.setScalar(7.5/horizontal);terrain.updateMatrixWorld(true);let box=new T.Box3().setFromObject(terrain),center=box.getCenter(new T.Vector3());terrain.position.x-=center.x;terrain.position.z-=center.z;terrain.updateMatrixWorld(true);box.setFromObject(terrain);terrain.position.y+=groundY-box.max.y;scene.add(terrain);$('source').textContent='REAL F450 + TERRAIN GLB';status('Terrain GLB loaded · F450 resting on launch field');log('Sketchfab terrain GLB loaded with unlit verified color texture')},undefined,()=>{status('Terrain GLB failed to load',true);log('Terrain failed: terrain.glb')})}
...
function setLabels(on){labelsVisible=!!on&&exploded;$('labels').classList.toggle('show',labelsVisible);$('labelsBtn').textContent=labelsVisible?'HIDE LABELS':'SHOW LABELS'}
function resetExplodedParts(){if(!exploded)return;parts.forEach(o=>{o.position.copy(o.userData.ex);o.userData.returning=false});status('Exploded parts reset to engineering layout')}
function setExploded(on){exploded=!!on;dragPart=null;parts.forEach(o=>{o.position.copy(exploded?o.userData.ex:o.userData.base);o.userData.returning=false});anchors.forEach(o=>o.position.copy(exploded?o.userData.ex:o.userData.base));overlay.visible=exploded;$('labelsBtn').disabled=!exploded;setLabels(false);updateRoutes();$('explode').textContent=exploded?'ASSEMBLE VIEW':'EXPLODE VIEW';status(exploded?'Exploded engineering view: reveal labels, then drag any part':'Assembled F450 view: propulsion ready')}
...
renderer.domElement.addEventListener('pointerdown',event=>{if(!exploded||event.button!==0)return;pointerAt(event);const hit=raycaster.intersectObjects(parts,true)[0];const part=hit&&rootPart(hit.object);if(!part)return;dragPart=part;controls.enabled=false;const world=part.getWorldPosition(new T.Vector3());dragPlane.setFromNormalAndCoplanarPoint(cam.getWorldDirection(new T.Vector3()),world);if(raycaster.ray.intersectPlane(dragPlane,dragPoint)){drone.worldToLocal(dragLocal.copy(dragPoint));dragOffset.copy(part.position).sub(dragLocal)}part.userData.returning=false;renderer.domElement.setPointerCapture(event.pointerId);renderer.domElement.classList.add('drag');status('Inspecting real F450 mesh · drag freely, release to return')});
...
$('live').onclick=()=>{status('Following live hub state');refreshTelemetry()};$('test').onclick=trigger;$('explode').onclick=()=>setExploded(!exploded);$('assemble').onclick=()=>setExploded(false);$('labelsBtn').onclick=()=>setLabels(!labelsVisible);$('resetParts').onclick=resetExplodedParts;addEventListener('resize',resize);resize();startPolling();
