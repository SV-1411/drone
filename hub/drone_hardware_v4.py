...
function prop(file,sdfBase,ex,dir,index){new ColladaLoader().load(U+file,d=>{d.scene.traverse(o=>{if(o.isMesh){o.material=mats.white;o.castShadow=o.receiveShadow=true}});const base=sdfPoint(sdfBase);d.scene.position.copy(base);d.scene.userData.base=base;d.scene.userData.ex=new T.Vector3(...ex);d.scene.userData.dir=dir;drone.add(d.scene);parts.push(d.scene);props.push(d.scene);tag(d.scene,'PROP '+index,'real Collada mesh','#dce7ed');loaded++;ready()},undefined,()=>{failed++;log('Prop failed: '+file);ready()})}
...
function ready(){if(loaded+failed!==10)return;const box=new T.Box3().setFromObject(drone),center=box.getCenter(new T.Vector3()),size=box.getSize(new T.Vector3()),max=Math.max(size.x,size.y,size.z)||1;drone.position.sub(center);cam.position.set(max*1.75,max*1.35,max*1.85);controls.target.set(0,0,0);controls.update();$('source').textContent=failed?'F450 PARTIAL':'REAL F450 MESH';status(failed?'F450 loaded with '+failed+' missing asset(s)':'All 10 real F450 mesh assets loaded');log('F450 assembly ready: '+loaded+'/10 real assets');makeEngineeringOverlay();setExploded(false)}
...
stl('base_link.stl',mats.frame,[0,0,0],[0,0,.78],'F450 FRAME','#39e6b8');
stl('leg_link.stl',mats.white,[0,0,.132],[0,-.68,-.75],'LANDING GEAR','#dfe5e8');
stl('battery_link.stl',mats.black,[-.025435,-.00022216,.09],[-.74,-.38,.18],'BATTERY','#ffb34c');
stl('front_arm_link.stl',mats.red,[.098091,.098091,.171],[.65,.2,.92],'FRONT ARM','#ef6570');
stl('back_arm_link.STL',mats.white,[-.0980914901943598,-.0980914949965026,.171],[-.65,.2,.92],'REAR ARM','#e6edf0');
stl('motor_link.STL',mats.black,[-.00077732,.0001817,.173],[0,.5,1.05],'MOTOR ASSEMBLY','#8de0cd');
...
let lastFrame=performance.now();function loop(now){requestAnimationFrame(loop);const dt=Math.min(.05,(now-lastFrame)/1000);lastFrame=now;const active=['ARMING','TAKEOFF','ENROUTE','HOVERING','DELIVERING','RTL','LANDING'].includes(state),effectiveRpm=active?Math.max(rpm,state==='ARMING'?1800:4300):0;currentAltitude+=(targetAltitude-currentAltitude)*Math.min(1,dt*4);drone.position.y=Math.min(currentAltitude*.035,1.8);const spin=effectiveRpm*Math.PI*2/60*dt;props.forEach(p=>p.rotation.y+=spin*(p.userData.dir||1));pulses.forEach(p=>{p.userData.t=(p.userData.t+p.userData.speed*dt)%1;const a=exploded?(p.userData.aEx||p.userData.aBase):p.userData.aBase;const b=exploded?(p.userData.bEx||p.userData.bBase):p.userData.bBase;p.position.lerpVectors(a,b,p.userData.t);p.visible=active});$('hud').textContent=state+' · '+Math.round(effectiveRpm)+' RPM · '+currentAltitude.toFixed(1)+' m AGL';controls.update();renderer.render(scene,cam);labels.render(scene,cam)}requestAnimationFrame(loop);
</script>
</body></html>'''
