...
def _get_asset(name: str):
    ...
    local_path = assets_dir / name if name == "terrain.glb" else assets_dir / "f450" / name
...
function loadTerrain(groundY){new GLTFLoader().load(U+'terrain.glb',gltf=>{const terrain=gltf.scene;terrain.traverse(o=>{if(!o.isMesh)return;o.castShadow=false;o.receiveShadow=true;const source=Array.isArray(o.material)?o.material[0]:o.material,map=source?.map;if(map){map.colorSpace=T.SRGBColorSpace;map.anisotropy=renderer.capabilities.getMaxAnisotropy();map.needsUpdate=true}o.material=new T.MeshLambertMaterial({map:map||null,color:0xffffff,side:T.DoubleSide})});terrain.updateMatrixWorld(true);const rawBox=new T.Box3().setFromObject(terrain),rawSize=rawBox.getSize(new T.Vector3()),horizontal=Math.max(rawSize.x,rawSize.z)||1;terrain.scale.setScalar(7.5/horizontal);terrain.updateMatrixWorld(true);let box=new T.Box3().setFromObject(terrain),center=box.getCenter(new T.Vector3());terrain.position.x-=center.x;terrain.position.z-=center.z;terrain.updateMatrixWorld(true);box.setFromObject(terrain);terrain.position.y+=groundY-box.max.y;scene.add(terrain);$('source').textContent='REAL F450 + TERRAIN GLB';status('Terrain GLB loaded · F450 resting on launch field');log('Sketchfab terrain GLB loaded with its embedded terrain texture')},undefined,()=>{status('Terrain GLB failed to load',true);log('Terrain failed: terrain.glb')})}
